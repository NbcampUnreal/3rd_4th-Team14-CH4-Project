...
public:
	...
	void ServerRPC_AddPickupItem(AITTestItemActor* ItemActorToPickup);

	UFUNCTION(BlueprintCallable, Server, Reliable, Category = "Inventory")
	void ServerRPC_RemoveItem(int32 SlotIndex);

	UFUNCTION(BlueprintCallable, Server, Reliable, Category = "Inventory")
	void ServerRPC_DropItem(int32 SlotIndex);

	UFUNCTION(BlueprintCallable, Server, Reliable, Category = "Inventory")
	void ServerRPC_UseItem(UITTestItemInstance* ItemInstance);

	UFUNCTION(BlueprintCallable, Server, Reliable, Category = "Inventory")
	void ServerRPC_ReduceItem(UITTestItemInstance* ItemInstance, int Quantity = 1);

	// TODO: Prototyping 함수, 퀵 슬롯 제작 후 이전
	UFUNCTION(BlueprintCallable, Server, Reliable, Category = "Inventory")
	void ServerRPC_TestUseItem(int32 SlotIndex);
	// 기존의 아이템 추가 함수 -> 테스트용 / 아이템 즉시 획득
	UFUNCTION(BlueprintCallable, Server, Reliable, Category = "Inventory")
	void ServerRPC_TestAddItem(UITTestItemData* ItemData, int32 AddQuantity);
...
protected:
	...
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, ReplicatedUsing = OnRep_InventoryUpdate, Category ="Inventory")
	int32 InventorySize = 4;

	UPROPERTY(ReplicatedUsing = OnRep_InventoryUpdate)
